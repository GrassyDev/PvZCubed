...
public class TrashcanEgg extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = PvZEntity.TRASHCAN.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
             if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
					BrowncoatEntity zombieEntity = (BrowncoatEntity) PvZEntity.TRASHCAN.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
