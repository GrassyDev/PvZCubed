...
public class SeashroomSeeds extends SeedItem implements FabricItem {
	public static int cooldown = (int) (PVZCONFIG.nestedSeeds.moreSeeds.seashroomS() * 20);
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("item.pvzmod.seashroom_seed_packet.flavour")
				.formatted(Formatting.DARK_GRAY));

		tooltip.add(Text.translatable("item.pvzmod.seashroom_seed_packet.flavour2")
				.formatted(Formatting.DARK_GRAY));
	}
		...
		if (hitResult.getType() == HitResult.Type.MISS) {
			...
			if (hitResult.getType() == HitResult.Type.BLOCK) {
				if (world instanceof ServerWorld) {
					SeashroomEntity aquaticEntity = this.createEntity(world, hitResult);
	...
	private SeashroomEntity createEntity(World world, HitResult hitResult) {
		return (SeashroomEntity) (new SeashroomEntity(world, hitResult.getPos().x, hitResult.getPos().y, hitResult.getPos().z));
	}
