...
public class ShootingBasketballEntity extends ThrownItemEntity implements IAnimatable {
    ...
    public ShootingBasketballEntity(EntityType<? extends ThrownItemEntity> entityType, World world) {
        super(entityType, world);
		this.setNoGravity(false);
    }

    public ShootingBasketballEntity(World world, LivingEntity owner) {
        super(EntityType.SNOWBALL, owner, world);
    }

    @Environment(EnvType.CLIENT)
    public ShootingBasketballEntity(World world, double x, double y, double z, float yaw, float pitch, int interpolation, boolean interpolate, int id, UUID uuid) {
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
		...
		if (!this.isHypno){
			if (!world.isClient && (!(entity instanceof Monster) || entity instanceof GeneralPvZombieEntity generalPvZombieEntity && generalPvZombieEntity.getHypno())) {
				entity.playSound(PvZCubed.PEAHITEVENT, 0.2F, (float) (0.5F + Math.random()));
				float damage = 4F;
