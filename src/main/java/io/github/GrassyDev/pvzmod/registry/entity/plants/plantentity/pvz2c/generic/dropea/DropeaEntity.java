...
public class DropeaEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
	...
	private static final TrackedData<Integer> DATA_ID_TYPE_VARIANT =
			DataTracker.registerData(DropeaEntity.class, TrackedDataHandlerRegistry.INTEGER);
	...
	protected void initGoals() {
		this.goalSelector.add(1, new DropeaEntity.FireBeamGoal(this));
	...
	static class FireBeamGoal extends Goal {
		private final DropeaEntity plantEntity;
		private int beamTicks;
		private int animationTicks;

		public FireBeamGoal(DropeaEntity plantEntity) {
			...
			if (!this.plantEntity.canSee(livingEntity)) {
				...
				if (this.beamTicks >= 0 && this.animationTicks <= -7) {
					if (!this.plantEntity.isInsideWaterOrBubbleColumn()) {
						ShootingDropEntity proj = new ShootingDropEntity(PvZEntity.DROP, this.plantEntity.world);
