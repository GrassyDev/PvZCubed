...
import static io.github.GrassyDev.pvzmod.registry.PvZEntity.FLAGPEASANT;

public class FlagPeasantEgg extends SeedItem {
    public FlagPeasantEgg(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = FLAGPEASANT.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
		if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				 double random = Math.random();
                    FlagPeasantEntity flagzombieEntity = (FlagPeasantEntity) FLAGPEASANT.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
