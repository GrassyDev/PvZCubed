...
public class BeeSpikeItem extends Item {
    public BeeSpikeItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.ENTITY_BEE_HURT, SoundCategory.NEUTRAL, 0.5F, 1F); // plays a globalSoundEvent

        if (!world.isClient) {
            ShootingBeeSpikeEntity proj = new ShootingBeeSpikeEntity(PvZEntity.BEESPIKE, world);
