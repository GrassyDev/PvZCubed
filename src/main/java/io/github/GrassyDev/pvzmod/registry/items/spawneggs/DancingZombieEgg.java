...
public class DancingZombieEgg extends SeedItem {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = PvZEntity.DANCINGZOMBIE.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
             if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    DancingZombieEntity dancingZombieEntity = (DancingZombieEntity) PvZEntity.DANCINGZOMBIE.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
                    ...
                    ((ServerWorld) world).spawnEntityAndPassengers(dancingZombieEntity);
					dancingZombieEntity.setPersistent();
                    world.playSound((PlayerEntity) null, dancingZombieEntity.getX(), dancingZombieEntity.getY(), dancingZombieEntity.getZ(), PvZSounds.ENTITYRISINGEVENT, SoundCategory.BLOCKS, 0.75F, 0.8F);
