...
public class GoldLeafEntity extends PlantEntity implements IAnimatable {

    private AnimationFactory factory = GeckoLibUtil.createFactory(this);
	private String controllerName = "nutcontroller";

	protected int turningTicks = 15;
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		...
		if (i <= 0) {
			event.getController().setAnimation(new AnimationBuilder().loop("goldleaf.idle"));
		}
		else {
			event.getController().setAnimation(new AnimationBuilder().loop("goldleaf.idle2"));
		}
	...
	public void tick() {
		...
		if (--turningTicks <=0 ){
			createGoldTile(this.getBlockPos());
			this.discard();
		}
	}
	...
	public ItemStack getPickBlockStack() {
		return ModItems.GOLDLEAF_SEED_PACKET.getDefaultStack();
	}
	...
	public static boolean canSmallnutSpawn(EntityType<? extends GoldLeafEntity> type, ServerWorldAccess world, SpawnReason spawnReason, BlockPos pos, RandomGenerator random) {
