...
public class CabbagepultSeeds extends SeedItem implements FabricItem {
    public boolean used;
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (direction == Direction.DOWN) {
			...
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				...
				List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.CABBAGEPULT.getDimensions().getBoxAt(plantEntity.getPos()));
				if (list.isEmpty()) {
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld) {
			plantEntity = PvZEntity.CABBAGEPULT.create(serverWorld, stack.getNbt(), (Text) null, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.CABBAGEPULT.getDimensions().getBoxAt(plantEntity.getPos()));
		}
