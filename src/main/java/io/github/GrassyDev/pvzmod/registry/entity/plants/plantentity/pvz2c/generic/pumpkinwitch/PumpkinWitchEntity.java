...
public class PumpkinWitchEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
	...
	public void writeCustomDataToNbt(NbtCompound tag) {
		...
		tag.putBoolean("Variant", this.hasProj());
	}
	...
	public void setProj(PumpkinWitchEntity.Proj proj) {
		this.dataTracker.set(DATA_ID_TYPE_VARIANT, proj.getId());
	}
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (!this.hasProj()){
			event.getController().setAnimation(new AnimationBuilder().loop("pumpkinwitch.idle2"));
		}
		else if (this.isFiring) {
			event.getController().setAnimation(new AnimationBuilder().playOnce("pumpkinwitch.throw"));
		}
		else {
			event.getController().setAnimation(new AnimationBuilder().loop("pumpkinwitch.idle"));
		}
	...
	protected void initGoals() {
		this.goalSelector.add(1, new PumpkinWitchEntity.FireBeamGoal(this));
	}
		...
		if (this.age > 1) {
			...
			if ((!blockPos2.equals(blockPos) || !blockState.hasSolidTopSurface(world, this.getBlockPos(), this)) && !this.hasVehicle()) {
				if (!this.world.isClient && this.world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT) && !this.naturalSpawn && this.age <= 10 && !this.dead){
					this.dropItem(ModItems.PUMPKINWITCH_SEED_PACKET);
				}
				this.discard();
			}
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (itemStack.isOf(ModItems.GARDENINGGLOVE)) {
			dropItem(ModItems.PUMPKINWITCH_SEED_PACKET);
	...
	public ItemStack getPickBlockStack() {
		return ModItems.PUMPKINWITCH_SEED_PACKET.getDefaultStack();
	}


	/** /~*~//~*ATTRIBUTES*~//~*~/ **/

	public static DefaultAttributeContainer.Builder createPumpkinWitchAttributes() {
	...
	static class FireBeamGoal extends Goal {
		private final PumpkinWitchEntity plantEntity;
		private int beamTicks;
		private int animationTicks;

		private boolean shot;
		public FireBeamGoal(PumpkinWitchEntity plantEntity) {
		...
		public boolean canStart() {
			...
			return livingEntity != null && livingEntity.isAlive() && this.plantEntity.hasProj();
		}

		public boolean shouldContinue() {
			return super.shouldContinue() && this.plantEntity.hasProj();
		}
		...
		public void stop() {
			if (shot){
				this.plantEntity.setProj(Proj.FALSE);
			}
		...
		public void tick() {
			...
			if ((!this.plantEntity.canSee(livingEntity)) &&
				...
				++this.beamTicks;
				++this.animationTicks;
				if (this.beamTicks >= 0 && this.animationTicks <= -7 && plantEntity.hasProj()) {
					...
					if (!this.plantEntity.isInsideWaterOrBubbleColumn() && this.plantEntity.hasProj()) {
						ShootingPumpkinEntity proj = new ShootingPumpkinEntity(PvZEntity.PUMPKINPROJ, this.plantEntity.world);
				...
				if (this.animationTicks >= -1) {
					this.plantEntity.recoverTicks = 500;
					this.plantEntity.setProj(Proj.FALSE);
				}
