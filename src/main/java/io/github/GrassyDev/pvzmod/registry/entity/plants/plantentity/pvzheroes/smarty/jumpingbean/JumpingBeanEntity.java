...
public class JumpingBeanEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.isFiring) {
			event.getController().setAnimation(new AnimationBuilder().playOnce("jumpingbean.attack"));
		}
		else if (this.getIsAsleep()){
			event.getController().setAnimation(new AnimationBuilder().loop("jumpingbean.tired"));
		}
		else {
			event.getController().setAnimation(new AnimationBuilder().loop("jumpingbean.idle"));
		}
	...
	protected void initGoals() {
		this.goalSelector.add(1, new JumpingBeanEntity.FireBeamGoal(this));
	...
	public ItemStack getPickBlockStack() {
		return ModItems.JUMPINGBEAN_SEED_PACKET.getDefaultStack();
	}
	...
	static class FireBeamGoal extends Goal {
		private final JumpingBeanEntity plantEntity;
		private int beamTicks;
		private int animationTicks;

		private boolean bounced;

		public FireBeamGoal(JumpingBeanEntity plantEntity) {
