...
public class ShamrockSeeds extends SeedItem implements FabricItem {
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (direction == Direction.DOWN) {
			...
			Box box = PvZEntity.SHAMROCK.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				ShamrockEntity plantEntity = (ShamrockEntity) PvZEntity.SHAMROCK.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
				List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.SHAMROCK.getDimensions().getBoxAt(plantEntity.getPos()));
				if (list.isEmpty()) {
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld) {
			plantEntity = PvZEntity.SHAMROCK.create(serverWorld, stack.getNbt(), (Text) null, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.SHAMROCK.getDimensions().getBoxAt(plantEntity.getPos()));
		}
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity
			...
			if (list.isEmpty()) {
				...
				double random = Math.random();
				if (random <= 0.25) {
					((ShamrockEntity) plantEntity).setVariant(ShamrockVariants.PRIDE);
				} else {
					((ShamrockEntity) plantEntity).setVariant(ShamrockVariants.DEFAULT);
				}
				world.spawnEntity(plantEntity);
