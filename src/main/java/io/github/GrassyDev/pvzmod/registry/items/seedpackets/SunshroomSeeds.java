	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (direction == Direction.DOWN) {
			...
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				SunshroomEntity sunshroomEntity = (SunshroomEntity) PvZEntity.SUNSHROOM.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
				List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.SUNSHROOM.getDimensions().getBoxAt(sunshroomEntity.getPos()));
				if (list.isEmpty()) {
					...
					sunshroomEntity.sunProducingTime = 100;
