...
public class BeautyshroomEntity extends PlantEntity implements IAnimatable {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
        ...
        if (this.getIsAsleep()){
            event.getController().setAnimation(new AnimationBuilder().loop("beautyshroom.idle.asleep"));
        }
        else if (i > 0) {
            event.getController().setAnimation(new AnimationBuilder().playOnce("beautyshroom.explosion"));
        } else {
            event.getController().setAnimation(new AnimationBuilder().loop("beautyshroom.idle"));
        }
