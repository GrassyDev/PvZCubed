...
public class BrickheadEgg extends SeedItem {
    public BrickheadEgg(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = PvZEntity.BRICKHEAD.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
             if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    ...
                    brickheadEntity.refreshPositionAndAngles(brickheadEntity.getX(), brickheadEntity.getY(), brickheadEntity.getZ(), f, 0.0F);
					brickheadEntity.setPersistent();
                    ((ServerWorld) world).spawnEntityAndPassengers(brickheadEntity);
                    world.playSound((PlayerEntity) null, brickheadEntity.getX(), brickheadEntity.getY(), brickheadEntity.getZ(), PvZCubed.ENTITYRISINGEVENT, SoundCategory.BLOCKS, 0.75F, 0.8F);
