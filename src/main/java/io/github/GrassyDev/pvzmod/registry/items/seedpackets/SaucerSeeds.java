...
public class SaucerSeeds extends Item implements FabricItem {
			...
			if (hitResult.getType() == HitResult.Type.BLOCK) {
				if (world instanceof ServerWorld) {
					...
					if (!world.isSpaceEmpty(aquaticEntity, aquaticEntity.getBoundingBox())) {
						...
						if (!world.isClient) {
							List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.SAUCER.getDimensions().getBoxAt(aquaticEntity.getPos()));
							List<TileEntity> list2 = world.getNonSpectatingEntities(TileEntity.class, PvZEntity.SAUCER.getDimensions().getBoxAt(aquaticEntity.getPos()));
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld) {
			...
			list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.SAUCER.getDimensions().getBoxAt(plantEntity.getPos()));
		}
