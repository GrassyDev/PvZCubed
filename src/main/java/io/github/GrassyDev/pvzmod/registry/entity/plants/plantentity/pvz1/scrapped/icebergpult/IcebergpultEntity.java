...
public class IcebergpultEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
	...
	protected void initGoals() {
		this.goalSelector.add(1, new IcebergpultEntity.FireBeamGoal(this));
		...
		if (this.age > 1) {
			...
			if ((!blockPos2.equals(blockPos) || !blockState.hasSolidTopSurface(world, this.getBlockPos(), this)) && !this.hasVehicle()) {
				if (!this.world.isClient && this.world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT) && !this.naturalSpawn && this.age <= 10 && !this.dead){
					this.dropItem(ModItems.ICEBERGPULT_SEED_PACKET);
				}
				this.discard();
			}
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (itemStack.isOf(ModItems.GARDENINGGLOVE)) {
			dropItem(ModItems.ICEBERGPULT_SEED_PACKET);
	...
	public ItemStack getPickBlockStack() {
		return ModItems.ICEBERGPULT_SEED_PACKET.getDefaultStack();
	}


	/** /~*~//~*ATTRIBUTES*~//~*~/ **/

	public static DefaultAttributeContainer.Builder createIcebergPultAttributes() {
	...
	static class FireBeamGoal extends Goal {
		private final IcebergpultEntity plantEntity;
		private int beamTicks;
		private int animationTicks;

		public FireBeamGoal(IcebergpultEntity plantEntity) {
