...
public class AdmiralNavyBeanSeeds extends Item implements FabricItem {
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("item.pvzmod.admiralnavybean_seed_packet.flavour")
				.formatted(Formatting.DARK_GRAY));

		tooltip.add(Text.translatable("item.pvzmod.admiralnavybean_seed_packet.flavour2")
				.formatted(Formatting.DARK_GRAY));
	}
			...
			if (hitResult.getType() == HitResult.Type.BLOCK) {
				if (world instanceof ServerWorld) {
					...
					if (!world.isSpaceEmpty(aquaticEntity, aquaticEntity.getBoundingBox())) {
						...
						if (!world.isClient) {
							List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.ADMIRALNAVYBEAN.getDimensions().getBoxAt(aquaticEntity.getPos()));
							List<TileEntity> list2 = world.getNonSpectatingEntities(TileEntity.class, PvZEntity.ADMIRALNAVYBEAN.getDimensions().getBoxAt(aquaticEntity.getPos()));
	...
	private AdmiralNavyBeanEntity createEntity(World world, HitResult hitResult) {
		return (AdmiralNavyBeanEntity) (new AdmiralNavyBeanEntity(world, hitResult.getPos().x, hitResult.getPos().y, hitResult.getPos().z));
	}
