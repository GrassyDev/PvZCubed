...
public class RoofGraveEntity extends GraveEntity implements IAnimatable {
	...
	public static DefaultAttributeContainer.Builder createRoofGraveAttributes() {
        return HostileEntity.createHostileAttributes().add(EntityAttributes.GENERIC_FOLLOW_RANGE, 100.0D)
                ...
                .add(EntityAttributes.GENERIC_MAX_HEALTH, 230D);
    }
        ...
        protected void castSpell() {
			...
			if (probability5 <= 0.15) { // 15% x1 Flag Zombie
				for (int f = 0; f < 1; ++f) {
					...
					BlockPos blockPos2 = RoofGraveEntity.this.getBlockPos().add(-2 + RoofGraveEntity.this.random.nextInt(5), 0.1, -2 + RoofGraveEntity.this.random.nextInt(5));
					BrowncoatEntity browncoatEntity = (BrowncoatEntity) PvZEntity.BROWNCOAT.create(RoofGraveEntity.this.world);
					...
					BlockPos blockPos6 = RoofGraveEntity.this.getBlockPos().add(-2 + RoofGraveEntity.this.random.nextInt(5), 0.1, -2 + RoofGraveEntity.this.random.nextInt(5));
			...
			if (difficulty >= 2.09) {
				if (probability6 <= 0.085) { // 8.5% x1/x2 Gargantuar
					int xx = 1;
					if (difficulty >= 2.39) {
						xx = 2;
					}
    ...
    class TrackOwnerTargetGoal extends TrackTargetGoal {
        ...
        public boolean canStart() {
            return RoofGraveEntity.this.owner != null && RoofGraveEntity.this.owner.getTarget() != null && this.canTrack(RoofGraveEntity.this.owner.getTarget(), this.TRACK_OWNER_PREDICATE);
        }

        public void start() {
            RoofGraveEntity.this.setTarget(RoofGraveEntity.this.owner.getTarget());
            super.start();
        }
    }
