...
public class PeasantKnightEgg extends SeedItem {
    public PeasantKnightEgg(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = PvZEntity.PEASANTKNIGHT.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
             if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
					PeasantEntity bucketheadEntity = (PeasantEntity) PvZEntity.PEASANTKNIGHT.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
                    ...
                    world.playSound((PlayerEntity) null, bucketheadEntity.getX(), bucketheadEntity.getY(), bucketheadEntity.getZ(), PvZSounds.ENTITYRISINGEVENT, SoundCategory.BLOCKS, 0.75F, 0.8F);
