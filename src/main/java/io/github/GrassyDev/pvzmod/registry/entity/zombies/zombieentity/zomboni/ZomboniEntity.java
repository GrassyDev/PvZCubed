...
public class ZomboniEntity extends ZombieRiderEntity implements IAnimatable {
	...
	private static final TrackedData<Integer> DATA_ID_TYPE_VARIANT =
			DataTracker.registerData(ZomboniEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    protected void initCustomGoals() {
		...
		this.targetSelector.add(5, new TargetGoal<>(this, MobEntity.class, 0, false, false, (livingEntity) -> {
			return livingEntity instanceof PlantEntity plantEntity && !(PLANT_LOCATION.get(plantEntity.getType()).orElse("normal").equals("flying"));
		}));
	...
	public static DefaultAttributeContainer.Builder createZomboniAttributes() {
        return HostileEntity.createHostileAttributes().add(EntityAttributes.GENERIC_FOLLOW_RANGE, 100.0D)
                .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.12D)
                .add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 4.0D)
                .add(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, 1.0D)
                .add(EntityAttributes.GENERIC_MAX_HEALTH, PVZCONFIG.nestedZombieHealth.zomboniH());
    }
	...
	public boolean damage(DamageSource source, float amount) {
        if (!super.damage(source, amount)) {
            ...
            if (this.getRecentDamageSource() == PvZCubed.HYPNO_DAMAGE && !(this.getHypno())) {
                ...
                ZomboniEntity hypnotizedZombie = (ZomboniEntity) hypnoType.create(world);
