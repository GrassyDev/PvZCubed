...
public class PeaPodSeeds extends SeedItem implements FabricItem {
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld) {
			plantEntity = PvZEntity.PEAPOD.create(serverWorld, stack.getNbt(), (Text) null, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.PEAPOD.getDimensions().getBoxAt(plantEntity.getPos()));
		}
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity
			...
			if (list.isEmpty()) {
				...
				double random = Math.random();
				if (random <= 0.25) {
					((PeapodEntity) plantEntity).setVariant(PeapodVariants.PLURAL);
				} else {
					((PeapodEntity) plantEntity).setVariant(PeapodVariants.DEFAULT);
				}
				world.spawnEntity(plantEntity);
