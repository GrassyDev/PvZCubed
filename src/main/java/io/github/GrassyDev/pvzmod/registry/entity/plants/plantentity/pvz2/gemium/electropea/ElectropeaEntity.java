...
public class ElectropeaEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
	...
	protected void initGoals() {
		this.goalSelector.add(1, new ElectropeaEntity.FireBeamGoal(this));
	}
		...
		if (this.age > 1) {
			...
			if ((!blockPos2.equals(blockPos) || !blockState.hasSolidTopSurface(world, this.getBlockPos(), this)) && !this.hasVehicle()) {
				if (!this.world.isClient && this.world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT) && !this.naturalSpawn && this.age <= 10 && !this.dead){
					this.dropItem(ModItems.ELECTROPEA_SEED_PACKET);
				}
				this.discard();
			}
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (itemStack.isOf(ModItems.GARDENINGGLOVE)) {
			dropItem(ModItems.ELECTROPEA_SEED_PACKET);
	...
	public ItemStack getPickBlockStack() {
		return ModItems.ELECTROPEA_SEED_PACKET.getDefaultStack();
	}


	/** /~*~//~*ATTRIBUTES*~//~*~/ **/

	public static DefaultAttributeContainer.Builder createElectropeaAttributes() {
	...
	static class FireBeamGoal extends Goal {
		private final ElectropeaEntity plantEntity;
		private int beamTicks;
		private int animationTicks;

		public FireBeamGoal(ElectropeaEntity plantEntity) {
			...
			if (!this.plantEntity.canSee(livingEntity)) {
				...
				if (this.beamTicks >= 0 && this.animationTicks <= -7) {
					if (!this.plantEntity.isInsideWaterOrBubbleColumn()) {
						ShootingElectricPeaEntity proj = new ShootingElectricPeaEntity(PvZEntity.ELECTRICPEA, this.plantEntity.world);
