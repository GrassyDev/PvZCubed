...
public class SuperFanImpEgg extends SeedItem {
    public SuperFanImpEgg(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = PvZEntity.SUPERFANIMP.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
		if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    SuperFanImpEntity impEntity = (SuperFanImpEntity) PvZEntity.SUPERFANIMP.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
                    if (impEntity == null) {
