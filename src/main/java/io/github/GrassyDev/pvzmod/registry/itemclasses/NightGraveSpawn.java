...
public class NightGraveSpawn extends Item {
    public NightGraveSpawn(Settings settings) {
        super(settings);
    }
        ...
        Box box = PvZEntity.NIGHTGRAVESTONE.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
            if (world.isSpaceEmpty((Entity) null, box, (entity) -> {
                ...
                if (world instanceof ServerWorld) {
                    ...
                    NightGraveEntity nightGraveEntity = (NightGraveEntity) PvZEntity.NIGHTGRAVESTONE.create(serverWorld, itemStack.getTag(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
                    if (nightGraveEntity == null) {
                        return ActionResult.FAIL;
                    }

                    serverWorld.spawnEntityAndPassengers(nightGraveEntity);
                    ...
                    world.playSound((PlayerEntity) null, nightGraveEntity.getX(), nightGraveEntity.getY(), nightGraveEntity.getZ(), ExampleMod.ENTITYRISINGEVENT, SoundCategory.BLOCKS, 0.75F, 0.8F);
                }
